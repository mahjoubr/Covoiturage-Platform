...
const CreatePostModal: React.FC<CreatePostModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    if (userData?.getAppUserInfo?.name && userData?.getAppUserInfo?.lastName) {
      setFormData((prev) => ({
        ...prev,
        driverName: `${userData.getAppUserInfo.name} ${userData.getAppUserInfo.lastName}`, // Combine name and lastName
      }));
    }
  }, [userData]);
    ...
    await createPost({
      variables: {
        createPostInput: {
          ...
          postOwnerId: userData.getAppUserInfo.id, // Replace with actual user ID dynamically if possible
